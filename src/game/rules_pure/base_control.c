...
int get_base_controller(const MatchSession* game, BaseID base)
{
    ...
    for (int i = 0; i < PLAYERS_IN_TEAM + JOKER_COUNT; i++) {
        if (game->referee.battingPlayers[i].currentSafetyBase == base && game->playerInfo[i].bTPI.baseId == base) {
...
int get_ball_at_base_index(const StateInfo* stateInfo)
{
    ...
    for (int i = 1; i < BASE_COUNT; i++) {
        Vector3D baseLoc;
        if (i == 1)
            baseLoc = stateInfo->fieldPositions->firstBase;
        else if (i == 2)
            baseLoc = stateInfo->fieldPositions->secondBase;
        else if (i == 3)
            baseLoc = stateInfo->fieldPositions->thirdBase;
        else
            continue;
