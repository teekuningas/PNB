...
void init_batting_system(StateInfo* stateInfo)
{
    ...
    stateInfo->match->pendingActionState.batting_frame_count = 0;
    stateInfo->match->pendingActionState.increase_batting_frame_count = 0;
    stateInfo->match->pendingActionState.selected_batting_power_count = 0;
    stateInfo->match->pendingActionState.selected_batting_angle_count = 0;
    stateInfo->match->pendingActionState.batter_angle = 0;
    stateInfo->match->pendingActionState.batter_angle_speed = 0;
    stateInfo->match->pendingActionState.batter_advance_speed = 0;
    stateInfo->match->pendingActionState.batter_advance = 0;
    stateInfo->match->pendingActionState.batting_mode = BATTING_MODE_SWING;
    stateInfo->match->pendingActionState.batter_advance_limit = 0;
    stateInfo->match->pendingActionState.batting_stopped = 0;
    stateInfo->match->pendingActionState.batter_moving = 0;
    stateInfo->match->pendingActionState.update_batter_location_and_orientation = 0;
    stateInfo->match->pendingActionState.pitch_frame_time = 0;
}

void start_increase_batter_angle(StateInfo* stateInfo)
{
    stateInfo->match->aF.bTAF.increase_batter_angle = ACTION_ACTIVE;
    // set batter_angle_speed to 1 to indicate that the direction of the movement is cw
    stateInfo->match->pendingActionState.batter_angle_speed = 1;
}
void stop_increase_batter_angle(StateInfo* stateInfo)
{
    stateInfo->match->aF.bTAF.increase_batter_angle = ACTION_IDLE;
    // when stopping the increasing of the angle, we want not to interrupt an ongoing decreasing of the angle
    if (stateInfo->match->pendingActionState.batter_angle_speed != -1) {
        stateInfo->match->pendingActionState.batter_angle_speed = 0;
    }
...
{
    stateInfo->match->aF.bTAF.decrease_batter_angle = ACTION_ACTIVE;
...
{
    stateInfo->match->aF.bTAF.decrease_batter_angle = ACTION_IDLE;
...
void select_batter(StateInfo* stateInfo)
{
    ...
    if (index != -1) {
        ...
        stateInfo->match->aF.bTAF.choose_batter = CHOOSE_BATTER_IDLE;
        ...
        // cant advance yet
        stateInfo->match->pRAI.batter_can_advance = 0;
...
void select_power(StateInfo* stateInfo)
{
    ...
    // 0.
    stateInfo->match->pendingActionState.selected_batting_power_count =
        stateInfo->match->pendingActionState.meter_counter - (BAT_SWING_MAX - BAT_LOAD_MAX);
    ...
    stateInfo->match->pendingActionState.meter_counter_max = BAT_SWING_MAX;

    // if power is small, we use bunting animation.
    if (stateInfo->match->pendingActionState.selected_batting_power_count < BUNT_THRESHOLD) {
        // and its updated here only if the player is moving already so that the animation wont start too early.
        if (stateInfo->match->pendingActionState.batter_moving == 1) {
            int batterIndex = get_active_batter_index(stateInfo->match);
            if (batterIndex != -1) {
        ...
        stateInfo->match->pendingActionState.batter_advance_limit = BUNT_ADVANCE;
        ...
        stateInfo->match->pendingActionState.batting_mode = BATTING_MODE_BUNT;
    }
...
void select_angle(StateInfo* stateInfo)
{
    ...
    stateInfo->match->pendingActionState.selected_batting_angle_count = stateInfo->match->pendingActionState.meter_counter;
}

void update_batting(StateInfo* stateInfo)
{
    int batterIndex = get_active_batter_index(stateInfo->match);
    if (stateInfo->match->pRAI.batter_ready == 1 && stateInfo->match->pRAI.pitch_state != PITCH_STAGE_AIRBORNE &&
        stateInfo->match->pRAI.batting_going_on == 0) {
        stateInfo->match->pRAI.batting_going_on = 1;
    }
    // so batting_going_on goes 1 when batter arrives to its ready position and pitch is not in air.
    if (stateInfo->match->pRAI.batting_going_on == 1) {
        // we increase batting_frame_count from the moment the batter movement started.
        // used for animation and advancing.
        if (stateInfo->match->pendingActionState.increase_batting_frame_count == 1) {
            stateInfo->match->pendingActionState.batting_frame_count += 1;
        }

        if (stateInfo->match->pRAI.init_batter == 1) {
            // so the default is to swing, we change this to BUNT_ADVANCE or SPREAD_ADVANCE
            // if the circumstances need so
            stateInfo->match->pendingActionState.batter_advance_limit = SWING_ADVANCE;
            // we start with natural speeds and positions
            stateInfo->match->pendingActionState.batter_angle = 0.0f;
            stateInfo->match->pendingActionState.batter_angle_speed = 0;
            stateInfo->match->pendingActionState.batter_advance = 0.0f;
            stateInfo->match->pendingActionState.batter_advance_speed = 0.0f;
            // aren't moving yet
            stateInfo->match->pendingActionState.batter_moving = 0;
            // swinging mode
            stateInfo->match->pendingActionState.batting_mode = BATTING_MODE_SWING;
            ...
            stateInfo->match->pendingActionState.update_batter_location_and_orientation = 1;
            if (batterIndex != -1) {
                stateInfo->match->playerInfo[batterIndex].cPI.lastLastLocationUpdate = 1;
            }
            // init done, so no need to do that anymore.
            stateInfo->match->pRAI.init_batter = 0;
        }
        ...
        if (stateInfo->match->pendingActionState.batter_angle_speed != 0 ||
            stateInfo->match->pendingActionState.batter_advance_speed != 0) {
            ...
            if (stateInfo->match->pendingActionState.batter_angle +
                        stateInfo->match->pendingActionState.batter_angle_speed * BATTER_ANGLE_SPEED_CONSTANT <
                    BATTER_ANGLE_LIMIT &&
                stateInfo->match->pendingActionState.batter_angle +
                        stateInfo->match->pendingActionState.batter_angle_speed * BATTER_ANGLE_SPEED_CONSTANT >
                    -BATTER_ANGLE_LIMIT) {
                stateInfo->match->pendingActionState.batter_angle +=
                    stateInfo->match->pendingActionState.batter_angle_speed * BATTER_ANGLE_SPEED_CONSTANT;
            ...
            if (stateInfo->match->pendingActionState.batter_advance +
                    stateInfo->match->pendingActionState.batter_advance_speed <
                stateInfo->match->pendingActionState.batter_advance_limit) {
                stateInfo->match->pendingActionState.batter_advance +=
                    stateInfo->match->pendingActionState.batter_advance_speed;
                stateInfo->match->pendingActionState.update_batter_location_and_orientation = 1;
            }
        }
        // if need for update of location and orientation
        if (stateInfo->match->pendingActionState.update_batter_location_and_orientation == 1 && batterIndex != -1) {
            float dx;
            float dz;
            float dx2;
            float dz2;
            ...
            set_vector_xz(
                ...
                (float)(stateInfo->fieldPositions->pitchPlate.x +
                        cos(ZERO_BATTING_ANGLE + stateInfo->match->pendingActionState.batter_angle) *
                            (BATTING_RADIUS * (1 - stateInfo->match->pendingActionState.batter_advance))),
                (float)(stateInfo->fieldPositions->pitchPlate.z -
                        sin(ZERO_BATTING_ANGLE + stateInfo->match->pendingActionState.batter_angle) *
                            (BATTING_RADIUS * (1 - stateInfo->match->pendingActionState.batter_advance)))
            );
        ...
        if (stateInfo->match->pRAI.pitch_state == PITCH_STAGE_AIRBORNE &&
            stateInfo->match->pendingActionState.run_bat_flag == 0) {
            // start the animation and advancing.
            if (stateInfo->match->pendingActionState.increase_batting_frame_count == 0) {
                stateInfo->match->pendingActionState.increase_batting_frame_count = 1;
            }
            ...
            if (stateInfo->match->aF.bTAF.swing == BAT_ACTION_IDLE) {
                ...
                stateInfo->match->pendingActionState.pitch_frame_time =
                ...
                stateInfo->match->pRAI.batter_ready = 0;
            }
            // so if the batter is still not moving, we'll try to figure out if we should be moving.
            if (stateInfo->match->pendingActionState.batter_moving == 0 && batterIndex != -1) {
                ...
                if (stateInfo->match->pendingActionState.batting_frame_count >
                    stateInfo->match->pendingActionState.pitch_frame_time - BAT_ANIMATION_FRAME_HIT_COUNT) {
                    // set batter_moving flag to 1 so that we can better handle starting points of the
                    // animations
                    stateInfo->match->pendingActionState.batter_moving = 1;
                    // so if we are still to swing
                    // select corresponding animation
                    if (stateInfo->match->pendingActionState.batting_mode == BATTING_MODE_SWING) {
                        stateInfo->match->playerInfo[batterIndex].cPI.model = PLAYER_ANIM_BAT_SWING_1;
                    }
                    // to bunt, select bunting animation
                    else if (stateInfo->match->pendingActionState.batting_mode == BATTING_MODE_BUNT) {
                        stateInfo->match->playerInfo[batterIndex].cPI.model = PLAYER_ANIM_BAT_SWING_2;
                    }
            ...
            if (stateInfo->match->aF.bTAF.swing == BAT_ACTION_WAIT_FOR_BALL) {
                if (stateInfo->match->pendingActionState.meter_counter - (BAT_SWING_MAX - BAT_LOAD_MAX) >=
                    BAT_LOAD_MAX) {
                    // change animation to hands spreading
                    if (stateInfo->match->pendingActionState.batter_moving == 1 && batterIndex != -1) {
                        stateInfo->match->playerInfo[batterIndex].cPI.model = PLAYER_ANIM_BAT_SWING_3;
                    }
                    ...
                    // starts.
                    stateInfo->match->pendingActionState.batting_mode = BATTING_MODE_STOP;
                    // advance only a small bit
                    stateInfo->match->pendingActionState.batter_advance_limit = SPREAD_ADVANCE;
                    // set flag to indicate that batting has stopped so that we there wont be checking for if the
                    // bat has hit the ball
                    stateInfo->match->pendingActionState.batting_stopped = 1;
        ...
        // so here we check if the bat hits. this event happens always the pitch has been in air
        else if (stateInfo->match->pendingActionState.batting_frame_count >
                 stateInfo->match->pendingActionState.pitch_frame_time) {
            ...
            if (stateInfo->match->pendingActionState.batting_stopped == 0 &&
                ...
                if (stateInfo->match->ballInfo.location.x < BALL_MAX_OFFSET &&
                    ...
                    verticalAngle = calculate_batting_vertical_angle(
                        stateInfo->match->pendingActionState.selected_batting_power_count,
                        stateInfo->match->pendingActionState.selected_batting_angle_count,
                    ...
                    horizontalAngle = -stateInfo->match->pendingActionState.batter_angle * 2;
                    power = (float)stateInfo->match->pendingActionState.selected_batting_power_count;
                    ...
                    if (verticalAngle > VERTICAL_ANGLE_LIMIT || verticalAngle < -VERTICAL_ANGLE_LIMIT) {
                        ...
                        if (batterIndex != -1) {
                            ...
                            if (stateInfo->match->pRAI.will_start_running[0] == 1) {
                                int base_index =
                                    get_base_controller(stateInfo->match, &stateInfo->rules->referee, BASE_HOME);
                                stateInfo->match->pRAI.will_start_running[0] = 0;
                                if (base_index != -1) {
                                    run_to_next_base(
                                        stateInfo->match, stateInfo->fieldPositions, base_index, BASE_HOME
                                    );
                                    stateInfo->match->pendingActionState.run_bat_flag = 1;
                                }
                            }
                        }
                    }
...
void update_batting_meter(StateInfo* stateInfo)
{
    ...
    if (stateInfo->match->aF.bTAF.swing == BAT_ACTION_WAIT_FOR_BALL) {
        ...
        stateInfo->match->pRAI.swing_meter_value = calculate_power_meter_value(
    ...
    // when power is selected but angle is to be selected
        ...
        stateInfo->match->pRAI.swing_meter_value = calculate_angle_meter_value(
            stateInfo->match->pendingActionState.meter_counter, stateInfo->match->pendingActionState.meter_counter_max,
            stateInfo->match->pendingActionState.selected_batting_power_count, BAT_SWING_MAX, BAT_LOAD_MAX
        );
    }
}
