...
static void changeBatter(StateInfo* stateInfo)
{
	...
	while(done == 0) {
		if(stateInfo->match->pendingActionState.batterSelect == 0) {
			if(stateInfo->match->playerCounters.nonJokerPlayersLeft != 0) done = 1;
			else stateInfo->match->pendingActionState.batterSelect = 1;
			...
			if(stateInfo->match->playerInfo[stateInfo->match->
			                                pII.jokerIndices[stateInfo->match->pendingActionState.batterSelect - 1]].bTPI.joker == JOKER_USED) stateInfo->match->pendingActionState.batterSelect++;
			else done = 1;
	...
	// player.
	if(stateInfo->match->pendingActionState.batterSelect == 0) {
		index = stateInfo->match->scoreboard.teams[battingTeamIndex].batterOrder[stateInfo->match->scoreboard.teams[battingTeamIndex].batterOrderIndex];
	} else {
				...
				if(stateInfo->match->pRAI.willStartRunning[base] == 0) {
					if(index != -1 &&stateInfo->match->playerInfo[index].cPI.moving == 0) {
						...
						if(base == BASE_FIRST || base == BASE_SECOND) {
							lead(stateInfo->match->playerInfo, stateInfo->match->playerRuntime, stateInfo->fieldPositions, index);
						}
					}
				} else {
...
static void updateMeters(StateInfo* stateInfo)
{
	...
	if(stateInfo->match->pendingActionState.throwGoingOn == 1) {
		...
		stateInfo->match->pRAI.meterValue = 1.0f*stateInfo->match->pendingActionState.meterCounter / stateInfo->match->pendingActionState.meterCounterMax;
	} else {
