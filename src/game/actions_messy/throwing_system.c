...
void prepareThrow(StateInfo* stateInfo, BaseID base)
{
	...
	case BASE_HOME:
		...
		stateInfo->match->pendingActionState.throwDirection.x = stateInfo->fieldPositions->pitcher.x - stateInfo->match->playerInfo[stateInfo->match->pII.hasBallIndex].tPI.location.x;
		stateInfo->match->pendingActionState.throwDirection.z = stateInfo->fieldPositions->pitcher.z - stateInfo->match->playerInfo[stateInfo->match->pII.hasBallIndex].tPI.location.z;
		break;
	case BASE_FIRST:
		stateInfo->match->pRAI.throwGoingToBase = 1;
		stateInfo->match->pendingActionState.throwDirection.x = stateInfo->fieldPositions->firstBase.x - stateInfo->match->playerInfo[stateInfo->match->pII.hasBallIndex].tPI.location.x;
		stateInfo->match->pendingActionState.throwDirection.z = stateInfo->fieldPositions->firstBase.z - stateInfo->match->playerInfo[stateInfo->match->pII.hasBallIndex].tPI.location.z;
		break;
	case BASE_SECOND:
		stateInfo->match->pRAI.throwGoingToBase = 2;
		stateInfo->match->pendingActionState.throwDirection.x = stateInfo->fieldPositions->secondBase.x - stateInfo->match->playerInfo[stateInfo->match->pII.hasBallIndex].tPI.location.x;
		stateInfo->match->pendingActionState.throwDirection.z = stateInfo->fieldPositions->secondBase.z - stateInfo->match->playerInfo[stateInfo->match->pII.hasBallIndex].tPI.location.z;
		break;
	case BASE_THIRD:
		stateInfo->match->pRAI.throwGoingToBase = 3;
		stateInfo->match->pendingActionState.throwDirection.x = stateInfo->fieldPositions->thirdBase.x - stateInfo->match->playerInfo[stateInfo->match->pII.hasBallIndex].tPI.location.x;
		stateInfo->match->pendingActionState.throwDirection.z = stateInfo->fieldPositions->thirdBase.z - stateInfo->match->playerInfo[stateInfo->match->pII.hasBallIndex].tPI.location.z;
		break;
	default:
		break;
	}
...
void genericThrowRelease(StateInfo* stateInfo)
{
	if(stateInfo->match->pII.hasBallIndex != -1) {
		...
		power = 1.0f*stateInfo->match->pendingActionState.meterCounter / stateInfo->match->pendingActionState.meterCounterMax;
		// update these values a bit
		stateInfo->match->pendingActionState.throwDirection.x = stateInfo->match->pendingActionState.throwDirection.x / stateInfo->match->pendingActionState.throwDistance;
		stateInfo->match->pendingActionState.throwDirection.z = stateInfo->match->pendingActionState.throwDirection.z / stateInfo->match->pendingActionState.throwDistance;
		stateInfo->match->pendingActionState.throwDirection.y = 0.06f;
		// ... and then launch the ball
		genericSlingBall(&(stateInfo->match->ballInfo), stateInfo->match->pendingActionState.throwDirection.x*power*THROW_POWER_CONSTANT, stateInfo->match->pendingActionState.throwDirection.y + stateInfo->match->pendingActionState.throwDistance*THROW_DISTANCE_CONSTANT, stateInfo->match->pendingActionState.throwDirection.z*power*THROW_POWER_CONSTANT);
...
void genericThrowLoad(StateInfo* stateInfo, BaseID base)
{
	if(stateInfo->match->pII.hasBallIndex != -1) {
		...
		stateInfo->match->pendingActionState.throwDistance = (float)sqrt(stateInfo->match->pendingActionState.throwDirection.x*stateInfo->match->pendingActionState.throwDirection.x + stateInfo->match->pendingActionState.throwDirection.z*stateInfo->match->pendingActionState.throwDirection.z);
		...
		if(stateInfo->match->pendingActionState.throwDistance > THROW_TO_BASE_DISTANCE) {
			...
			stateInfo->match->pendingActionState.meterCounterMax = THROW_MAX; // arbitrary decision, seems about right though
			...
			stateInfo->match->playerInfo[stateInfo->match->pII.hasBallIndex].tPI.orientation.x = stateInfo->match->pendingActionState.throwDirection.x;
			stateInfo->match->playerInfo[stateInfo->match->pII.hasBallIndex].tPI.orientation.z = stateInfo->match->pendingActionState.throwDirection.z;
		} else {
...
void genericMove(StateInfo* stateInfo, int direction)
{
	...
	if(stateInfo->match->pendingActionState.throwGoingOn == 0 &&stateInfo->match->pRAI.pitchState == PITCH_STAGE_NONE &&
	        stateInfo->match->pII.controlIndex != -1) {
...
void dropBall(StateInfo* stateInfo)
{
	...
	if(stateInfo->match->pII.hasBallIndex != -1) {
		if(stateInfo->match->pendingActionState.throwGoingOn == 0 &&stateInfo->match->pRAI.pitchState == PITCH_STAGE_NONE) {
			...
			genericSlingBall(&(stateInfo->match->ballInfo), dx*DROP_BALL_CONSTANT, DROP_BALL_CONSTANT, dz*DROP_BALL_CONSTANT);
...
void updateControlledPlayerSpeed(StateInfo* stateInfo)
{
	if(stateInfo->match->pII.controlIndex != -1) {
		...
		if(stateInfo->match->playerInfo[stateInfo->match->pII.controlIndex].cTPI.throwRecoil == 0) {
			...
			int directionZ = - stateInfo->match->playerInfo[stateInfo->match->pII.controlIndex].cTPI.movesToDirection[0] +
			...
			if(directionX*directionX + directionZ*directionZ == 0) {
				...
				if(stateInfo->match->pII.hasBallIndex == stateInfo->match->pII.controlIndex)
					stateInfo->match->playerInfo[stateInfo->match->pII.controlIndex].cPI.model = PLAYER_ANIM_STAND_WITH_BALL;
				else
				...
				// running
				stateInfo->match->playerInfo[stateInfo->match->pII.controlIndex].tPI.velocity.x = (float)directionX*RUN_SPEED/norm;
				stateInfo->match->playerInfo[stateInfo->match->pII.controlIndex].tPI.velocity.z = (float)directionZ*RUN_SPEED/norm;
				...
				if(stateInfo->match->pII.hasBallIndex == stateInfo->match->pII.controlIndex) {
					stateInfo->match->playerInfo[stateInfo->match->pII.controlIndex].cPI.model = PLAYER_ANIM_RUN_WITH_BALL;

				} else {
					stateInfo->match->playerInfo[stateInfo->match->pII.controlIndex].cPI.model = PLAYER_ANIM_RUN_NO_BALL;
				}
			}
		}
	}
