...
void selectBatter(StateInfo* stateInfo)
{
	...
	if(index != -1) {
		...
		if(stateInfo->match->playerInfo[index].bTPI.joker == JOKER_AVAILABLE) {
			...
			stateInfo->match->scoreboard.teams[battingTeamIndex].batterOrderIndex = (stateInfo->match->scoreboard.teams[battingTeamIndex].batterOrderIndex + 1)%PLAYERS_IN_TEAM;
		}
...
void selectPower(StateInfo* stateInfo)
{
	...
	stateInfo->match->pendingActionState.selectedBattingPowerCount = stateInfo->match->pendingActionState.meterCounter - (BAT_SWING_MAX - BAT_LOAD_MAX);
...
{
	int batterIndex = get_active_batter_index(stateInfo->match);
	if(stateInfo->match->pRAI.batterReady == 1 &&stateInfo->match->pRAI.pitchState != PITCH_STAGE_AIRBORNE &&stateInfo->match->pRAI.battingGoingOn == 0) {
	...
	if(stateInfo->match->pRAI.battingGoingOn == 1) {
		...
		if(stateInfo->match->pendingActionState.batterAngleSpeed != 0 || stateInfo->match->pendingActionState.batterAdvanceSpeed != 0) {
			...
			if(stateInfo->match->pendingActionState.batterAngle + stateInfo->match->pendingActionState.batterAngleSpeed*BATTER_ANGLE_SPEED_CONSTANT <
			        BATTER_ANGLE_LIMIT &&stateInfo->match->pendingActionState.batterAngle + stateInfo->match->pendingActionState.batterAngleSpeed*BATTER_ANGLE_SPEED_CONSTANT >
			        -BATTER_ANGLE_LIMIT ) {
				stateInfo->match->pendingActionState.batterAngle += stateInfo->match->pendingActionState.batterAngleSpeed*BATTER_ANGLE_SPEED_CONSTANT;
				stateInfo->match->pendingActionState.updateBatterLocationAndOrientation = 1;
			}
			...
			if(stateInfo->match->pendingActionState.batterAdvance + stateInfo->match->pendingActionState.batterAdvanceSpeed < stateInfo->match->pendingActionState.batterAdvanceLimit) {
				stateInfo->match->pendingActionState.batterAdvance += stateInfo->match->pendingActionState.batterAdvanceSpeed;
				stateInfo->match->pendingActionState.updateBatterLocationAndOrientation = 1;
			}
		}
		...
		if(stateInfo->match->pendingActionState.updateBatterLocationAndOrientation == 1 &&batterIndex != -1) {
			...
			setVectorXZ(&(stateInfo->match->playerInfo[batterIndex].tPI.lastLocation),
			...
			setVectorXZ(&(stateInfo->match->playerInfo[batterIndex].tPI.location),
			            (float)(stateInfo->fieldPositions->pitchPlate.x + cos(ZERO_BATTING_ANGLE + stateInfo->match->pendingActionState.batterAngle)*(BATTING_RADIUS *
			                    (1 - stateInfo->match->pendingActionState.batterAdvance))),
			            (float)(stateInfo->fieldPositions->pitchPlate.z - sin(ZERO_BATTING_ANGLE + stateInfo->match->pendingActionState.batterAngle)*(BATTING_RADIUS *
			                    (1 - stateInfo->match->pendingActionState.batterAdvance))));
		...
		if(stateInfo->match->pRAI.pitchState == PITCH_STAGE_AIRBORNE &&stateInfo->match->pendingActionState.runBatFlag == 0) {
			...
			if(stateInfo->match->aF.bTAF.swing == BAT_ACTION_IDLE) {
				...
				stateInfo->match->pendingActionState.pitchFrameTime = calculate_pitch_frame_time(v, GRAVITY, 0.0f, PITCH_FRAME_TIME_TWEAK);
			...
			if(stateInfo->match->pendingActionState.batterMoving == 0 &&batterIndex != -1) {
				...
				if(stateInfo->match->pendingActionState.battingFrameCount > stateInfo->match->pendingActionState.pitchFrameTime - BAT_ANIMATION_FRAME_HIT_COUNT) {
					...
					// for clarity
					stateInfo->match->pendingActionState.batterAdvanceSpeed = (float)GENERIC_BATTER_ADVANCE_SPEED_CONSTANT/BAT_ANIMATION_FRAME_HIT_COUNT;
				}
			}
			...
			if(stateInfo->match->aF.bTAF.swing == BAT_ACTION_WAIT_FOR_BALL) {
				if(stateInfo->match->pendingActionState.meterCounter - (BAT_SWING_MAX - BAT_LOAD_MAX) >= BAT_LOAD_MAX) {
		...
		if((stateInfo->match->pendingActionState.battingFrameCount > stateInfo->match->pendingActionState.pitchFrameTime -
		...
		else if(stateInfo->match->pendingActionState.battingFrameCount > stateInfo->match->pendingActionState.pitchFrameTime) {
			// so here we continue only if user hasn't decided to not to bat and if we havent bat already.
			if(stateInfo->match->pendingActionState.battingStopped == 0 &&stateInfo->match->pRAI.batHit == 0 &&stateInfo->match->pRAI.batMiss == 0) {
				// if ball doesnt go too far away to left or right
				if(stateInfo->match->ballInfo.location.x < BALL_MAX_OFFSET &&stateInfo->match->ballInfo.location.x > -BALL_MAX_OFFSET) {
					float verticalAngle;
					float horizontalAngle;
					float power;

					verticalAngle = calculate_batting_vertical_angle(stateInfo->match->pendingActionState.selectedBattingPowerCount, stateInfo->match->pendingActionState.selectedBattingAngleCount, stateInfo->match->ballInfo.velocity.y, BAT_SWING_MAX, BAT_LOAD_MAX);
					...
					if(verticalAngle > VERTICAL_ANGLE_LIMIT || verticalAngle < - VERTICAL_ANGLE_LIMIT) {
						...
						if (batterIndex != -1) {
							powerFactor = stateInfo->match->
							              playerInfo[batterIndex].bTPI.power;

							velocity = calculate_batted_ball_velocity(verticalAngle, horizontalAngle, power, powerFactor, stateInfo->match->ballInfo.location.x);
...
void updateBattingMeter(StateInfo* stateInfo)
{
	...
	if(stateInfo->match->aF.bTAF.swing == BAT_ACTION_WAIT_FOR_BALL) {
		...
		stateInfo->match->pRAI.swingMeterValue = calculate_power_meter_value(stateInfo->match->pendingActionState.meterCounter, stateInfo->match->pendingActionState.meterCounterMax);
	}
	// when power is selected but angle is to be selected
		...
		// otherwise select angle to be the maximum.
			...
			stateInfo->match->pendingActionState.selectedBattingAngleCount = stateInfo->match->pendingActionState.meterCounterMax;
		}

		stateInfo->match->pRAI.swingMeterValue = calculate_angle_meter_value(stateInfo->match->pendingActionState.meterCounter, stateInfo->match->pendingActionState.meterCounterMax, stateInfo->match->pendingActionState.selectedBattingPowerCount, BAT_SWING_MAX, BAT_LOAD_MAX);
	}
}
