...
void gameManipulation(StateInfo* stateInfo)
{
    ...
    ); // update ball's location due to velocity, also update ball's velocity due to gravity, and set some flags related
       // to ball
    ...
    playerLocationOrientationAndTargets(
        stateInfo
...
static void checkIfBallCanBeCatched(StateInfo* stateInfo)
{
    ...
    if (stateInfo->match->pII.hasBallIndex == -1 && stateInfo->match->pRAI.pitchState == PITCH_STAGE_NONE &&
        ...
        for (i = PLAYERS_IN_TEAM + JOKER_COUNT; i < PLAYERS_IN_TEAM * 2 + JOKER_COUNT; i++) {
            ...
            if ((i != stateInfo->match->pII.lastHadBallIndex ||
                 stateInfo->match->ballInfo.currentFlightHasHitGround == 1)) {
                Vector3D distance;
                float limit = CATCH_DISTANCE;
                distance.x = stateInfo->match->playerInfo[i].tPI.location.x - stateInfo->match->ballInfo.location.x;
                distance.z = stateInfo->match->playerInfo[i].tPI.location.z - stateInfo->match->ballInfo.location.z;
                distance.y =
                    2 * stateInfo->match->playerInfo[i].tPI.location.y / 5 - stateInfo->match->ballInfo.location.y;
                ...
                if (isVectorSmallEnoughSphere(&distance, limit) == 1) {
                    ...
                    if (stateInfo->match->pII.lastHadBallIndex != -1) {
                        ...
                        p2x = stateInfo->match->playerInfo[stateInfo->match->pII.lastHadBallIndex].tPI.location.x;
                        p2z = stateInfo->match->playerInfo[stateInfo->match->pII.lastHadBallIndex].tPI.location.z;
                        if (stateInfo->match->pRAI.throwGoingToBase != -1 &&
                            (i == stateInfo->match->pII.catcherOnBaseIndex[stateInfo->match->pRAI.throwGoingToBase] ||
                             i == stateInfo->match->pII
                                      .catcherReplacerOnBaseIndex[stateInfo->match->pRAI.throwGoingToBase])) {
                            baseCatcherFlag = 1;
                        }
                    }
                    // to avoid annyoing twitching we need to check that catching player is far enough
                    if (stateInfo->match->pII.lastHadBallIndex == -1 ||
                        stateInfo->match->ballInfo.currentFlightHasHitGround == 1 || baseCatcherFlag == 1 ||
                        ...
                        if (stateInfo->match->pII.controlIndex != -1 &&
                            stateInfo->match->playerInfo[stateInfo->match->pII.controlIndex].cTPI.throwRecoil == 0) {
                        ...
                        setVectorV(
                            &(stateInfo->match->ballInfo.location), &(stateInfo->match->playerInfo[i].tPI.location)
                        );
...
static void checkIfNearHomeLocation(StateInfo* stateInfo)
{
    ...
    for (i = PLAYERS_IN_TEAM + JOKER_COUNT; i < PLAYERS_IN_TEAM * 2 + JOKER_COUNT; i++) {
        ...
        if (stateInfo->match->playerInfo[i].cPI.moving == 1) {
            ...
            float dx =
                stateInfo->match->playerInfo[i].tPI.location.x - stateInfo->match->playerInfo[i].tPI.homeLocation.x;
            float dz =
                stateInfo->match->playerInfo[i].tPI.location.z - stateInfo->match->playerInfo[i].tPI.homeLocation.z;
    ...
    if (stateInfo->match->gameEvents.playerArrivedAtBase == 1) {
        ...
        for (i = 0; i < PLAYERS_IN_TEAM + JOKER_COUNT; i++) {
            ...
            if (stateInfo->match->playerInfo[index].bTPI.baseId != BASE_NONE &&
                stateInfo->match->playerRuntime[index].arrivedToBase == 1) {
...
static void basemenReplacements(StateInfo* stateInfo)
{
    ...
    for (i = 0; i < BASE_COUNT; i++) {
        ...
        if (stateInfo->match->playerInfo[stateInfo->match->pII.catcherOnBaseIndex[i]].cTPI.isNearHomeLocation == 0) {
            ...
            if (stateInfo->match->pII.catcherReplacerOnBaseIndex[i] != stateInfo->match->pII.controlIndex) {
                if (stateInfo->match->playerInfo[stateInfo->match->pII.catcherReplacerOnBaseIndex[i]]
                    ...
                    stateInfo->match->playerInfo[stateInfo->match->pII.catcherReplacerOnBaseIndex[i]]
                        .cTPI.replacingStage = REPLACEMENT_ACTIVE;
                    stateInfo->match->playerInfo[stateInfo->match->pII.catcherReplacerOnBaseIndex[i]]
        ...
        else if (stateInfo->match->playerInfo[stateInfo->match->pII.catcherOnBaseIndex[i]].cTPI.isNearHomeLocation ==
                 1) {
...
static void rankPlayersAndMoveThem(StateInfo* stateInfo)
{
    ...
    if (stateInfo->match->pRAI.refreshCatchAndChange == 1 && stateInfo->match->pII.hasBallIndex == -1) {
        ...
        if (stateInfo->match->ballInfo.currentFlightHasHitGround == 0) {
            float finalPointXApprox =
                ballDropX + stateInfo->match->ballInfo.velocity.x * BALL_FINAL_POINT_APPROXIMATION_CONSTANT;
            float finalPointZApprox =
                ballDropZ + stateInfo->match->ballInfo.velocity.z * BALL_FINAL_POINT_APPROXIMATION_CONSTANT;
            ...
            evalBallX =
                ballDropX + stateInfo->match->ballInfo.velocity.x * BALL_DROP_EVAL_CONSTANT * 0.25f / (time * 0.01f);
            evalBallZ =
                ballDropZ + stateInfo->match->ballInfo.velocity.z * BALL_DROP_EVAL_CONSTANT * 0.25f / (time * 0.01f);
        } else if (stateInfo->match->ballInfo.currentFlightHasHitGround == 1 &&
                   stateInfo->match->ballInfo.onGround == 0) {
            stateInfo->match->cameraState.targetPoint.x =
                ballDropX + stateInfo->match->ballInfo.velocity.x * BALL_DROP_TO_FINAL_POINT_APPROXIMATION_CONSTANT;
            stateInfo->match->cameraState.targetPoint.z =
                ballDropZ + stateInfo->match->ballInfo.velocity.z * BALL_DROP_TO_FINAL_POINT_APPROXIMATION_CONSTANT;
...
static void playerLocationOrientationAndTargets(StateInfo* stateInfo)
{
    ...
    for (i = 0; i < PLAYERS_IN_TEAM * 2 + JOKER_COUNT; i++) {
        ...
        if (stateInfo->match->ballInfo.moving == 1 || stateInfo->match->playerInfo[i].cPI.moving == 1) {
    ...
    for (i = 0; i < PLAYERS_IN_TEAM * 2 + JOKER_COUNT; i++) {
        ...
        if (stateInfo->match->playerInfo[i].cPI.moving == 1) {
            setVectorXZ(
                &stateInfo->match->playerInfo[i].tPI.lastLocation, stateInfo->match->playerInfo[i].tPI.location.x,
            ...
            if (i >= PLAYERS_IN_TEAM + JOKER_COUNT) {
                if ((stateInfo->match->playerInfo[i].tPI.location.z > FIELD_FRONT - FENCE_OFFSET &&
                     stateInfo->match->playerInfo[i].tPI.velocity.z > 0) ||
                    (stateInfo->match->playerInfo[i].tPI.location.z < FIELD_BACK + FENCE_OFFSET &&
                     stateInfo->match->playerInfo[i].tPI.velocity.z < 0)) {
                    stateInfo->match->ballInfo.needsMoveUpdate = 1;
                    stateInfo->match->playerInfo[i].tPI.velocity.z = 0.0f;
                }
                if ((stateInfo->match->playerInfo[i].tPI.location.x > FIELD_RIGHT - FENCE_OFFSET &&
                     stateInfo->match->playerInfo[i].tPI.velocity.x > 0) ||
                    (stateInfo->match->playerInfo[i].tPI.location.x < FIELD_LEFT + FENCE_OFFSET &&
                     stateInfo->match->playerInfo[i].tPI.velocity.x < 0)) {
            ...
            addToVectorXZ(
                &stateInfo->match->playerInfo[i].tPI.location, stateInfo->match->playerInfo[i].tPI.velocity.x,
            ...
            if (stateInfo->match->playerInfo[i].bTPI.state == PLAYER_STATE_WOUNDED) {
                ...
                // %.2f)\n", i, stateInfo->match->playerInfo[i].tPI.location.x,
                // stateInfo->match->playerInfo[i].tPI.location.z, stateInfo->match->playerInfo[i].tPI.velocity.x,
                // stateInfo->match->playerInfo[i].tPI.velocity.z, stateInfo->match->playerInfo[i].tPI.targetLocation.x,
                // stateInfo->match->playerInfo[i].tPI.targetLocation.z);
            }
            ...
            if (stateInfo->match->playerInfo[i].cPI.looksForTarget == 1) {
                ...
                float dx = stateInfo->match->playerInfo[i].tPI.location.x -
                           stateInfo->match->playerInfo[i].tPI.targetLocation.x;
                    ...
                    if (i < PLAYERS_IN_TEAM + JOKER_COUNT) {
                        ...
                        if ((stateInfo->match->playerInfo[i].bTPI.state == PLAYER_STATE_OUT) ||
                            ...
                            if (stateInfo->match->playerRuntime[i].passedPathPoint == 0) {
                                ...
                                stopTargetLookingPlayer(
                                    stateInfo->match->playerInfo, stateInfo->match->playerRuntime, i
                                );
                                setOrientation(stateInfo->match->playerInfo, &(stateInfo->match->ballInfo), i);
                                continue;
                            }
                        ...
                        // if we are legally running bases
                            ...
                            if (base_can_advance(stateInfo->match->playerInfo[i].bTPI.baseId) &&
                                ...
                                if (stateInfo->match->playerRuntime[i].goingForward == 1) {
                                    ...
                                    stateInfo->match->playerInfo[i].bTPI.baseId =
                                        base_get_next(stateInfo->match->playerInfo[i].bTPI.baseId);
...
static void updateModels(StateInfo* stateInfo)
{
    ...
    for (i = 0; i < PLAYERS_IN_TEAM * 2 + JOKER_COUNT; i++) {
        ...
        case PLAYER_ANIM_RUN_WITH_BALL:
            ...
            stateInfo->match->playerInfo[i].cPI.animationStage =
                (stateInfo->match->playerInfo[i].cPI.animationStage + 1) %
                (stateInfo->match->playerInfo[i].cPI.animationFrequency *
                 stateInfo->match->playerInfo[i].cPI.animationStageCount);
            break;
        ...
        case PLAYER_ANIM_THROW_WINDUP:
            ...
            if (stateInfo->match->playerInfo[i].cPI.animationStage !=
                stateInfo->match->playerInfo[i].cPI.animationStageCount *
                        stateInfo->match->playerInfo[i].cPI.animationFrequency -
                    1) {
                stateInfo->match->playerInfo[i].cPI.animationStage =
    ...
    if (ballAtBase == 0) {
        ...
        if (stateInfo->match->gameFlowState.homeRunCameraCounter == -1 &&
            ...
            (get_base_controller(stateInfo->match, BASE_THIRD) == -1 ||
             stateInfo->match->playerInfo[get_base_controller(stateInfo->match, BASE_THIRD)].bTPI.state ==
