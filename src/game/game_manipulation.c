...
static void updateBallStatus(MatchSession* match, GameFlowState* gameFlowState);
static void checkIfBallCanBeCatched(MatchSession* match, const FieldPositions* field_positions, int* play_sound_effect);
static void playerLocationOrientationAndTargets(MatchSession* match, const FieldPositions* field_positions);
static void basemenReplacements(MatchSession* match);
static void checkIfNearHomeLocation(MatchSession* match);
static void baseRunnerMovementsOnBaseArrivals(MatchSession* match);
static void moveIdlingPlayersToHomeLocation(MatchSession* match);
static void rankPlayersAndMoveThem(MatchSession* match);
static void updateModels(MatchSession* match);
static void updateBallHomeStatus(MatchSession* match, const FieldPositions* field_positions);

void game_manipulation(MatchSession* match, const FieldPositions* field_positions, int* play_sound_effect)
{
    updateBallStatus(match, &(match->gameFlowState)); // update ball's location due to velocity, also update ball's
                                                      // velocity due to gravity, and set some flags related to ball
    checkIfBallCanBeCatched(match, field_positions, play_sound_effect);
    checkIfNearHomeLocation(match);
    basemenReplacements(match);
    moveIdlingPlayersToHomeLocation(match);
    ...
    baseRunnerMovementsOnBaseArrivals(match); // Process arrival events generated by location update immediately
    updateBallToPlayer(
        match
    ); // if some player has the ball, update ball velocity so that the ball will follow the player
    updateBallHomeStatus(match, field_positions); // NEW: Check if ball is at home
    updateModels(match);
}
                ...
                if (isVectorSmallEnoughSphere(&distance, limit) == 1) {
                    ...
                    if (match->pII.lastHadBallIndex == -1 || match->ballInfo.currentFlightHasHitGround == 1 ||
                        ...
                        *play_sound_effect = SOUND_CATCH;
                        ...
                        setVectorV(&(match->ballInfo.location), &(match->playerInfo[i].tPI.location));
            ...
            if (match->playerInfo[index].bTPI.baseId != BASE_NONE && match->playerRuntime[index].arrivedToBase == 1) {
                ...
                if (match->playerInfo[index].bTPI.state != PLAYER_STATE_OUT) {
                    ...
                    if (match->playerInfo[index].bTPI.baseId == BASE_HOME_SCORED) {
                        ...
                        if (get_base_controller(match, BASE_THIRD) == index) {
                        }
            ...
            if (i != match->pII.hasBallIndex) {
                if (match->playerInfo[i].cPI.running == 0) {
                    if (i != activeBatterIndex) {
                        setOrientation(match->playerInfo, &(match->ballInfo), i);
                    }
                }
            }
        }
                    ...
                    if (i < PLAYERS_IN_TEAM + JOKER_COUNT) {
                        ...
                        if ((match->playerInfo[i].bTPI.state == PLAYER_STATE_OUT) ||
                            ...
                            if (match->playerRuntime[i].passedPathPoint == 0) {
                                ...
                                stopTargetLookingPlayer(match->playerInfo, match->playerRuntime, i);
                                setOrientation(match->playerInfo, &(match->ballInfo), i);
                                continue;
                            }
                            // and move to target
...
static void updateBallHomeStatus(MatchSession* match, const FieldPositions* field_positions)
{
    ...
    if (ballAtBase == 0) {
        ...
        if (match->gameFlowState.homeRunCameraCounter == -1 && match->cameraState.homeRunCameraFlag == 1 &&
            (get_base_controller(match, BASE_THIRD) == -1 ||
             match->playerInfo[get_base_controller(match, BASE_THIRD)].bTPI.state == PLAYER_STATE_ON_BASE)) {
