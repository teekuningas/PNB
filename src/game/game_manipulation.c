...
static void base_runner_movements_on_base_arrivals(MatchSession* match);
...
static void update_ball_home_status(MatchSession* match, const FieldPositions* field_positions);
...
void game_manipulation(MatchSession* match, const FieldPositions* field_positions, int* play_sound_effect)
{
    ...
    base_runner_movements_on_base_arrivals(match); // Process arrival events generated by location update immediately
    update_ball_to_player(
        match
    ); // if some player has the ball, update ball velocity so that the ball will follow the player
    update_ball_home_status(match, field_positions); // NEW: Check if ball is at home
    update_models(match);
}
