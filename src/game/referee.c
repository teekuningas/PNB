...
static void update_tuplahaava_logic(const StateInfo* stateInfo, RefereeState* referee, HalfInningState* halfInningState, int ballAtBase)
{
	...
	for (int i = 0; i < PLAYERS_IN_TEAM + JOKER_COUNT; i++) {
		...
		if (referee->battingPlayers[i].status == PLAYER_STATUS_WOUND_PENDING_DOUBLE) {
			...
			int is_in_between = (player->bTPI.state == PLAYER_STATE_RUNNING ||
			                     player->bTPI.state == PLAYER_STATE_LEADING);
			...
			} else if (player->bTPI.state == PLAYER_STATE_ON_BASE ||
			           player->bTPI.state == PLAYER_STATE_AT_BAT) {
...
int is_player_marked_for_wound(const RefereeState* ref, int playerIndex)
{
	...
	return (ref->battingPlayers[playerIndex].status == PLAYER_STATUS_WOUND_MARKED ||
