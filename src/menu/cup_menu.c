...
static MenuStage updateScreen_Initial(CupMenuState* cupMenuState, StateInfo* stateInfo, const KeyStates* keyStates)
{
    ...
    if (keyStates->released[0][KEY_UP]) {
        cupMenuState->initial.pointer =
            (cupMenuState->initial.pointer - 1 + cupMenuState->initial.rem) % cupMenuState->initial.rem;
    }
...
static MenuStage
updateScreen_Ongoing(CupMenuState* cupMenuState, StateInfo* stateInfo, CupMenuOutput* output, unsigned int* rng_seed)
{
    ...
    if (cupMenuState->ongoing.pointer == 0) { // "Next day"
        ...
        for (int i = 0; i < match_count; i++) {
            ...
            if (match->team_a_id == stateInfo->cup->user_team_id || match->team_b_id == stateInfo->cup->user_team_id) {
...
static MenuStage updateScreen_NewCup(
    CupMenuState* cupMenuState, StateInfo* stateInfo, const KeyStates* keyStates, unsigned int* rng_seed
)
{
    if (cupMenuState->new_cup.new_cup_stage == NEW_CUP_STAGE_TEAM_SELECTION) {
        if (keyStates->released[0][KEY_DOWN])
            cupMenuState->new_cup.pointer = (cupMenuState->new_cup.pointer + 1) % cupMenuState->new_cup.rem;
        if (keyStates->released[0][KEY_UP])
            cupMenuState->new_cup.pointer =
                (cupMenuState->new_cup.pointer - 1 + cupMenuState->new_cup.rem) % cupMenuState->new_cup.rem;
        if (keyStates->released[0][KEY_2]) {
        ...
        if (keyStates->released[0][KEY_2]) {
            ...
            if (cupMenuState->new_cup.pointer == 0)
                innings_per_period = 2;
            else if (cupMenuState->new_cup.pointer == 1)
                innings_per_period = 4;
            else
                innings_per_period = 8;
            ...
            stateInfo->cup = cup_create(
                stateInfo->numTeams, cupMenuState->new_cup.wins_to_advance, cupMenuState->new_cup.team_selection,
                innings_per_period, initial_team_ids
            );
...
{
    if (keyStates->released[0][KEY_DOWN])
        cupMenuState->load_save.pointer = (cupMenuState->load_save.pointer + 1) % cupMenuState->load_save.rem;
    if (keyStates->released[0][KEY_UP])
        cupMenuState->load_save.pointer =
            (cupMenuState->load_save.pointer - 1 + cupMenuState->load_save.rem) % cupMenuState->load_save.rem;
    if (keyStates->released[0][KEY_1]) {
        cupMenuState->screen = CUP_MENU_SCREEN_INITIAL;
...
static void drawScreen_Initial(
    const CupInitialState* initialState, const StateInfo* stateInfo, const RenderState* rs, ResourceManager* rm
)
{
    ...
    if (stateInfo->cup != NULL) {
        draw_text_2d(
            "Resume", center_x, menu_start_y + (menu_line++ * menu_spacing), menu_fontsize, TEXT_ALIGN_CENTER, rs
        );
    }
...
static void drawScreen_Ongoing(
    const CupOngoingState* ongoingState, const StateInfo* stateInfo, const RenderState* rs, ResourceManager* rm
)
{
    const float center_x = VIRTUAL_WIDTH / 2.0f;
    const float title_y = VIRTUAL_HEIGHT * 0.1f;
...
static void drawScreen_NewCup(
    const CupNewState* newCupState, const StateInfo* stateInfo, const RenderState* rs, ResourceManager* rm
)
{
    const float center_x = VIRTUAL_WIDTH / 2.0f;
    const float title_y = VIRTUAL_HEIGHT * 0.1f;
    ...
    if (newCupState->new_cup_stage == NEW_CUP_STAGE_TEAM_SELECTION) {
        ...
        for (int i = 0; i < stateInfo->numTeams; i++) {
            draw_text_2d(
                stateInfo->teamData[i].name, center_x, menu_start_y + i * menu_spacing, menu_fontsize,
                TEXT_ALIGN_CENTER, rs
            );
        }
...
static void drawScreen_ViewTree(
    const CupViewTreeState* viewTreeState, const StateInfo* stateInfo, const RenderState* rs, ResourceManager* rm
)
{
    ...
    for (int i = 0; i < stateInfo->cup->num_matches; i++) {
        ...
        if (match->team_a_id != -1) {
            ...
            draw_text_2d(
                stateInfo->teamData[match->team_a_id].name, team_name_x, text_y, text_size, TEXT_ALIGN_CENTER, rs
            );
            char wins_str[8];
        ...
        if (match->team_b_id != -1) {
            ...
            float text_y = text_center_y - text_size / 2.0f;
            draw_text_2d(
                stateInfo->teamData[match->team_b_id].name, team_name_x, text_y, text_size, TEXT_ALIGN_CENTER, rs
            );
            char wins_str[8];
...
static void drawScreen_ViewSchedule(
...
)
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (match->team_a_id != -1 && match->team_b_id != -1) {
            ...
            draw_text_2d(
                stateInfo->teamData[match->team_a_id].name, center_x - team_name_offset, current_y, text_fontsize,
                TEXT_ALIGN_CENTER, rs
            );
            draw_text_2d("-", center_x, current_y, text_fontsize, TEXT_ALIGN_CENTER, rs);
            draw_text_2d(
                stateInfo->teamData[match->team_b_id].name, center_x + team_name_offset, current_y, text_fontsize,
                TEXT_ALIGN_CENTER, rs
            );
        }
    }
...
static void drawScreen_LoadOrSaveCup(
    const CupLoadSaveState* loadSaveState, const CupMenuScreen screen, const CupMenuState* cupMenuState,
    const StateInfo* stateInfo, const RenderState* rs, ResourceManager* rm
)
{
    const float center_x = VIRTUAL_WIDTH / 2.0f;
    const float title_y = VIRTUAL_HEIGHT * 0.1f;
    ...
    for (int i = 0; i < 5; i++) {
        ...
        if (slot_info->exists) {
            ...
            if (slot_info->is_finished) {
                ...
                sprintf(
                    details, "Day %d - Next: %s", slot_info->current_day + 1,
...
static void drawScreen_EndCredits(const CreditsMenuState* creditsState, const RenderState* rs, ResourceManager* rm)
{
    ...
    draw_texture_2d(
        resource_manager_get_texture(rm, "data/textures/menu_trophy.tga"), trophy_x, trophy_y, trophy_size, trophy_size
    );

    // --- Draw Text ---
    draw_text_2d("WE HAVE A CHAMPION!", center_x, title_y, title_fontsize, TEXT_ALIGN_CENTER, rs);

    const char* credits_text = "SPECIAL THANKS TO JUUSO HEINILA, PEKKA HEINILA, PETRI ANTTILA, MATTI PITKANEN, VILLE "
                               "VILJANMAA, PETRI MIKOLA, TUOMAS NURMELA, MIKKO SYRJÄNEN AND OTHERS..";
        ...
        if (saved_cup != NULL) {
            ...
            if (saved_cup->matches[0].winner_id != CUP_MATCH_NO_WINNER) {
                ...
                if (user_match_index != -1) {
                    ...
                    TeamID opponent_id =
                        (match->team_a_id == saved_cup->user_team_id) ? match->team_b_id : match->team_a_id;
...
)
{
    ...
    case CUP_MENU_SCREEN_ONGOING: {
        ...
        if (keyStates->released[0][KEY_DOWN])
            cupMenuState->ongoing.pointer = (cupMenuState->ongoing.pointer + 1) % cupMenuState->ongoing.rem;
        if (keyStates->released[0][KEY_UP])
            cupMenuState->ongoing.pointer =
                (cupMenuState->ongoing.pointer - 1 + cupMenuState->ongoing.rem) % cupMenuState->ongoing.rem;
