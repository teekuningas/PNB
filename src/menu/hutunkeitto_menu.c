...
)
{
    if (state->state == 0) {
        ...
        float turnHeight = (HAND_WIDTH * (1.0f + SCALE_FACTOR * state->leftScaleCount) +
                            HAND_WIDTH * (1.0f + SCALE_FACTOR * state->rightScaleCount)) /
                           2;
        state->batTimer += 1;
        state->batHeight =
            BAT_DEFAULT_HEIGHT + state->batTimerCount * BAT_DROP_MOVING_SPEED + state->batTimer * BAT_MOVING_SPEED;
...
)
{
    ...
    // referee hand
    glBindTexture(
        GL_TEXTURE_2D, resource_manager_get_texture(rm, "data/textures/team2.tga")
    ); // Referee has a default texture
    glPushMatrix();
