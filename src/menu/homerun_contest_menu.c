...
)
{
    ...
    if (state->player_control == 2) {
        ...
        finish_selection();
        return (currentStage == MENU_STAGE_HOMERUN_CONTEST_1) ? MENU_STAGE_HOMERUN_CONTEST_2 : MENU_STAGE_GO_TO_GAME;
    }
    ...
    if (keyStates->released[state->player_control][KEY_2]) {
        if (state->pointer == 0) { // "Continue" selected
            if (state->choiceCounter >= state->choiceCount) {
                finish_selection();
                return (currentStage == MENU_STAGE_HOMERUN_CONTEST_1) ? MENU_STAGE_HOMERUN_CONTEST_2
