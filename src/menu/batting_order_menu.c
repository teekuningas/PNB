...
void initBattingOrderState(BattingOrderState* state, int team_index, int player_control, const StateInfo* stateInfo)
{
    ...
    for (int i = 0; i < PLAYERS_IN_TEAM + JOKER_COUNT; i++) {
        ...
        strncpy(
            state->players[i].name, stateInfo->teamData[team_index].players[i].name, sizeof(state->players[i].name) - 1
        );
...

{
    ...
    for (int i = 0; i < PLAYERS_IN_TEAM + JOKER_COUNT; i++) {
        ...
        draw_text_2d(
            state->players[player_index].name, col_name_x, list_start_y + i * option_spacing, text_size,
            TEXT_ALIGN_LEFT, rs
        );
        char stat_str[2];
    ...
    draw_texture_2d(
        resource_manager_get_texture(rm, "data/textures/arrow.tga"), current_arrow_x, arrow_y, arrow_size, arrow_size
    );
    ...
    if (state->mark != 0) {
        ...
        draw_texture_2d(
            resource_manager_get_texture(rm, "data/textures/arrow.tga"), marked_arrow_x, marked_arrow_y, arrow_size,
            arrow_size
        );
    }
}
