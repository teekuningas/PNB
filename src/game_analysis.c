					...
					if(index != -1) {
						...
						if(stateInfo.localGameInfo->pII.safeOnBaseIndex[i] == index) {
							if(stateInfo.localGameInfo->playerInfo[index].bTPI.isOnBase == 0) {
								...
								// to next base
								runToNextBase(stateInfo.localGameInfo->pII.safeOnBaseIndex[i], i);
...
static __inline void strikesAndBalls()
{
	...
	if(stateInfo.localGameInfo->gAI.strikes == 3) {
		...
		if(stateInfo.localGameInfo->pII.safeOnBaseIndex[0] != -1) {
			runToNextBase(stateInfo.localGameInfo->pII.safeOnBaseIndex[0], 0);
...
static __inline void checkIfEndOfInning()
{
	...
	if(endOfInningCounter > 200) {
		...
		if(stateInfo.screen != 0) loadMutableWorldSettings();
	}
}
