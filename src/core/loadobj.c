...
int load_obj(const char* path, const char* objectname, MeshObject* meshObj)
{
    ...
    if (objectname != NULL) {
        ...
        if (retval == 1) return OBJ_NAME_NOT_FOUND;
    }
    ...
    if (uFaceList == NULL || fPositionIndex == NULL || fNormalIndex == NULL || fTexCoordIndex == NULL) {
        free(uFaceList);
        free(fPositionIndex);
        free(fNormalIndex);
        free(fTexCoordIndex);
    ...
    while (1) {
        ...
        if (fgets(line, MAX_LINE, objfile) != NULL) {
            ...
            if (objectname != NULL) {
                if (param[0] == 'o') break; // Break out if it's a new object.
            }
            ...
            case 'v': // One of three other types.
                ...
                case ' ': // Regular vertex.
                    sscanf(
                        line, "v %f %f %f", &fPositionIndex[vcnt + 0], &fPositionIndex[vcnt + 1],
                        &fPositionIndex[vcnt + 2]
                    );
                    vcnt += 3;
                    break;
                case 'n': // Vertex normal.
                    sscanf(
                        line, "vn %f %f %f", &fNormalIndex[vncnt + 0], &fNormalIndex[vncnt + 1],
                        &fNormalIndex[vncnt + 2]
                    );
                    vncnt += 3;
                    break;
                case 't': // Texture coord.
                    sscanf(line, "vt %f %f", &fTexCoordIndex[vtcnt + 0], &fTexCoordIndex[vtcnt + 1]);
                    vtcnt += 2;
                    break;
                }
                break;
            case 'f':
                if (bNoNormals) {
                    sscanf(
                        line, "f %d//%d %d//%d %d//%d", &uFaceList[fcnt + 0], &uFaceList[fcnt + 3],
                    ...
                    sscanf(
                        line, "f %d/%d/%d %d/%d/%d %d/%d/%d", &uFaceList[fcnt + 0], &uFaceList[fcnt + 3],
                        &uFaceList[fcnt + 1], &uFaceList[fcnt + 4], &uFaceList[fcnt + 7], &uFaceList[fcnt + 5],
