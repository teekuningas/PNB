/*
    provides means to write on the screen and also provides the background that main menu uses and statistics in game
   screen use. had to put them in same place because you cannot load same texture in multiple places. the basic idea is
   just that we render planes one at a time for each character that have texture of a font image, and texture
   coordinates are the things that change. lights and z buffer should be off and we should be drawing last. also here i
   use this in perspective camera mode so we have to have specific kind of camera matrix.
*/
