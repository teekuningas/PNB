...
void cup_update_match_result(Cup* cup, int match_index, TeamID winner_team_id)
{
    ...
    if (match->winner_id != CUP_MATCH_NO_WINNER) {
        fprintf(
            stderr, "Warning: Match %d already has a winner (%d). Ignoring result.\n", match_index, match->winner_id
        );
        return;
    }
    ...
    if (match->team_a_id == winner_team_id) {
        ...
        fprintf(
            stderr, "Error: Winner team ID %d does not match teams in match %d (%d vs %d).\n", winner_team_id,
            match_index, match->team_a_id, match->team_b_id
        );
        return;
    }
    ...
    if (match_winner_id != CUP_MATCH_NO_WINNER) {
        ...
        if (parent_match_index != -1) {
            ...
            if (match_index == (2 * parent_match_index + 1)) { // Left child
                ...
                fprintf(
                    stderr, "Error: Match %d is not a child of parent %d. Logic error.\n", match_index,
                    parent_match_index
                );
            }
        }
