...
int test_fly_ball_double_wound_late_arrival(void)
{
    ...
    for (int frame = 0; frame <= 800; frame++) {
        ...
        if (frame % 20 == 0 || stateA == PLAYER_STATE_WOUNDED || stateB == PLAYER_STATE_WOUNDED ||
            arrivalAFrame == frame) {
            printf(
                "  F%3d: A[St=%d Bs=%d Sf=%d Status=%d] B[St=%d Bs=%d Sf=%d Status=%d] Tmr=%d\n", frame, stateA, baseA,
                safeA, statusA, stateB, baseB, safeB, statusB, timer
            );
        }
    ...
    printf(
        "  TUPLAHAAVA TRACE END: Catch=%d, TimerExp=%d, ArrA=%d, WoundA=%d, WoundB=%d\n", catchFrame, timerExpiredFrame,
        arrivalAFrame, woundAFrame, woundBFrame
    );
