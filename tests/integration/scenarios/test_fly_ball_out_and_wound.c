...
/*
  Rules for Koppi:
   ...
   3. Out (Palo):
       ...
       - This implies that if the ball is caught (fly), AND then thrown to the target base before the runner arrives, it's an OUT.
...
int test_full_fly_ball_out_and_wound(void)
{
	...
	printf("  2nd Base: (%.1f, %.1f, %.1f)\n",
	       ctx->state->fieldPositions->secondBase.x,
	       ctx->state->fieldPositions->secondBase.y,
	...
	for (int frame = 0; frame < 1200; frame++) {
		...
		if (match->playerInfo[1].bTPI.baseId == BASE_FIRST && batterArrivalFrame == -1) {
			...
			printf("Frame %d: Batter arrived at 1st (state=%d, safety=%d)\n",
			       frame, match->playerInfo[1].bTPI.state, safetyBase);
		}
		...
		if (!batterLostBase && match->playerInfo[1].bTPI.baseId == BASE_NONE) {
			batterLostBase = 1;
			int safetyBase = match->referee.battingPlayers[1].currentSafetyBase;
			printf("Frame %d: Batter lost base (baseId=NONE, safety=%d, state=%d)\n",
			       frame, safetyBase, match->playerInfo[1].bTPI.state);
		}
		if (!runnerLostBase && match->playerInfo[0].bTPI.baseId == BASE_NONE) {
			...
			printf("Frame %d: Runner A lost base (baseId=NONE, safety=%d, state=%d)\n",
			       frame, safetyBase, match->playerInfo[0].bTPI.state);
		}
		...
		if (frame % 20 == 0 || frame == catchFrame || frame == throwStartFrame ||
			...
			printf("  F%3d: RunnerA[St=%d Bs=%d Pos=(%.1f,%.1f)] BatterB[St=%d Bs=%d Pos=(%.1f,%.1f)] Ball[AtBase=%d]\n",
			       frame,
			       match->playerInfo[0].bTPI.state, match->playerInfo[0].bTPI.baseId,
			       match->playerInfo[0].tPI.location.x, match->playerInfo[0].tPI.location.z,
			       match->playerInfo[1].bTPI.state, match->playerInfo[1].bTPI.baseId,
			       match->playerInfo[1].tPI.location.x, match->playerInfo[1].tPI.location.z,
			       ballAtBase
			      );
		}
	}
	...
	printf("\n=== FINAL STATES ===\n");
	printf("Batter: state=%d, baseId=%d, safety=%d, position=(%.1f, %.1f, %.1f)\n",
	       match->playerInfo[1].bTPI.state,
	       match->playerInfo[1].bTPI.baseId,
	       match->referee.battingPlayers[1].currentSafetyBase,
	       match->playerInfo[1].tPI.location.x,
	       match->playerInfo[1].tPI.location.y,
	       match->playerInfo[1].tPI.location.z);
	printf("Runner A: state=%d, baseId=%d, safety=%d, position=(%.1f, %.1f, %.1f)\n",
	       ...
	       match->playerInfo[0].tPI.location.y,
	       match->playerInfo[0].tPI.location.z);
	printf("\nFor reference:\n");
	printf("  Home: (%.1f, %.1f, %.1f)\n",
	       ctx->state->fieldPositions->pitchPlate.x,
	       ctx->state->fieldPositions->pitchPlate.y,
	       ctx->state->fieldPositions->pitchPlate.z);
	printf("  1st Base: (%.1f, %.1f, %.1f)\n",
	       ctx->state->fieldPositions->firstBase.x,
	       ctx->state->fieldPositions->firstBase.y,
	       ctx->state->fieldPositions->firstBase.z);
	printf("  2nd Base: (%.1f, %.1f, %.1f)\n",
	       ctx->state->fieldPositions->secondBase.x,
	       ctx->state->fieldPositions->secondBase.y,
	       ctx->state->fieldPositions->secondBase.z);
