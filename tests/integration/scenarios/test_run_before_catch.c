...
int test_run_arrival_before_catch(void)
{
	...
	for (int i = 0; i < 600; i++) {
		...
		int hadPendingWound = ctx->state->match->referee.battingPlayers[runnerIndex].hasPendingWound;
		...
		int hasPendingWoundNow = ctx->state->match->referee.battingPlayers[runnerIndex].hasPendingWound;
		...
		if (ctx->state->match->playerInfo[runnerIndex].bTPI.baseId == BASE_HOME_SCORED && !arrived) {
			arrived = 1;
			arrivalFrame = i;
			printf("Frame %d: Runner arrived at HOME (baseId=4, state=%d)\n",
			       i, ctx->state->match->playerInfo[runnerIndex].bTPI.state);
		}
		...
		if (ctx->state->match->referee.woundingEvaluationActive && !woundingStarted) {
			...
			printf("Frame %d: Wounding evaluation STARTED (timer=%d, active=%d)\n",
			       i, ctx->state->match->referee.woundingEvaluationTimer,
			       ctx->state->match->referee.woundingEvaluationActive);
		}
		...
		if (ctx->state->match->referee.woundingEvaluationFinished && !woundingFinished) {
			...
			printf("Frame %d: Wounding evaluation FINISHED (finished=%d)\n",
			...
			printf("  Runner state=%d, hasPendingWound=%d, hasPendingRun=%d\n",
			       ...
			       ctx->state->match->referee.battingPlayers[runnerIndex].hasPendingWound,
		...
		if (ctx->state->match->playerInfo[runnerIndex].bTPI.state == PLAYER_STATE_WOUNDED && !runnerWounded) {
			runnerWounded = 1;
			woundFrame = i;
			printf("Frame %d: Runner WOUNDED (state=%d, baseId=%d)\n",
			       i, ctx->state->match->playerInfo[runnerIndex].bTPI.state,
			       ctx->state->match->playerInfo[runnerIndex].bTPI.baseId);
		}
	...
	if (pendingRunSet) {
		printf("Frame %d: Pending run SET (delta: %+d frames from arrival)\n",
		       pendingRunSetFrame, pendingRunSetFrame - arrivalFrame);
	} else {
	...
	if (woundingStarted) {
		printf("Frame %d: Wounding evaluation started (delta: %+d frames from catch)\n",
		       woundingStartFrame, woundingStartFrame - catchFrame);
	}
	if (woundingFinished) {
		printf("Frame %d: Wounding evaluation finished (delta: %+d frames from start)\n",
		       woundingFinishFrame, woundingFinishFrame - woundingStartFrame);
	}
	if (pendingRunCleared) {
		printf("Frame %d: Pending run CLEARED (delta: %+d frames from evaluation finish)\n",
		       pendingRunClearedFrame, pendingRunClearedFrame - woundingFinishFrame);
	} else {
		printf("ERROR: Pending run was NEVER cleared!\n");
	}
	if (runnerWounded) {
		printf("Frame %d: Runner wounded (delta: %+d frames from evaluation finish)\n",
		       woundFrame, woundFrame - woundingFinishFrame);
	} else {
