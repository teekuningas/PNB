...
int test_is_run_of_honor()
{
    ...
    ASSERT_EQ(
        0, is_run_of_honor(BASE_THIRD, BASE_FIRST, 0, 0), "Player starting from 1st base cannot score run of honor"
    );
    ASSERT_EQ(
        0, is_run_of_honor(BASE_THIRD, BASE_SECOND, 0, 0), "Player starting from 2nd base cannot score run of honor"
    );

    // Already made run of honor
    ASSERT_EQ(
        0, is_run_of_honor(BASE_THIRD, BASE_HOME, 0, 1), "Cannot score run of honor if already made on this base visit"
    );
